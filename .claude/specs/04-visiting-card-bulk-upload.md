...
## Overview
Implements the first workflow step in the DASHR AI pipeline (capture → extraction → enrichment → scoring → review/export): letting a logged-in seller select or create an exhibition, drag/drop dozens–hundreds of visiting-card photos in one batch, and have each one persisted as a `visiting_cards` row (`status = 'new'`) with its image durably stored in object storage. This step owns upload, validation, storage, and async queueing plumbing only — it does **not** implement OCR/extraction itself (that's the next step, `05-card-extraction`). Per CLAUDE.md, the upload request must never block on OCR, so extraction always runs as a Celery task, never inline; the task body is a placeholder this step stands up so the next step only has to fill in extraction logic, not build the queue.

**Amendment (post-`05-parsing-visiting-card`)**: uploading a batch does **not** automatically enqueue extraction anymore. `POST /cards/bulk-upload` only creates the `status='new'` rows and stores the images — parsing is a separate, explicit action the caller triggers afterward via `POST /cards/process` (see `05-parsing-visiting-card`'s spec for that endpoint's full behavior). This lets a rep upload a batch, review/re-order/discard before committing to the (billed) vision-extraction step, rather than every upload silently spending API calls the moment files land. Every mention below of the upload endpoint "enqueuing" a task describes the original `04` behavior and is superseded by this amendment.
...
- `POST /cards/bulk-upload` — org-authenticated — `multipart/form-data`: `exhibition_id` (optional form field, UUID) + `files` (one or more image files). Accepted formats: JPEG, PNG, WEBP, and HEIC/HEIF (`settings.allowed_card_image_content_types`, decoded/verified via Pillow + the `pillow-heif` plugin — HEIC/HEIF is what iPhones produce by default, and exhibition photos are overwhelmingly taken on phone cameras). For each file: validates content-type/size, uploads to object storage, creates a `visiting_cards` row (`user_id = current_user.user_id`, `exhibition_id` if provided, `status = 'new'`, `original_filename`, `image_url` = storage key). Returns `201` with `{ batch_size, cards: [{ card_id, original_filename, status, exhibition_id }] }`. If `exhibition_id` is provided but doesn't belong to the caller (or their org, if admin), returns `404` and creates nothing. If any file fails validation (unsupported type or over the size limit) or the batch exceeds the max file count, the **entire request is rejected with `400`** before any file is uploaded or any row is created — no partial batches, no orphaned storage objects. **Does not enqueue extraction** (see Amendment above) — cards sit at `status='new'` until `POST /cards/process` (added in `05-parsing-visiting-card`) is called.
...
- **New**: `app/upload/page.tsx` — authenticated bulk-upload page (`<Sidebar active="upload"/>`, same `min-h-screen bg-white flex` shell as `dashboard/page.tsx`/`profile/page.tsx`). Lets the user pick an existing exhibition from a dropdown (populated via `GET /exhibitions`) or create a new one inline (`POST /exhibitions`), then drag-drop or file-pick multiple images, submit via `POST /cards/bulk-upload`, and see a per-file success/failure summary followed by the resulting card list (filename + status) pulled via `GET /cards`. The file-pick `<input>`'s `accept` attribute is `"image/*,.heic,.heif"`, not just `"image/*"` — some mobile browsers/OS file pickers don't reliably recognize `image/heic` as matching an `image/*` wildcard, so the explicit extensions are needed for HEIC photos to even appear as selectable. **Amendment**: a "Parse Cards" button appears above the card list whenever any listed card has `status='new'`, calling `POST /cards/process` (see `05-parsing-visiting-card`) — parsing is never triggered automatically by the upload itself.
...
- New task: `apps/api/app/workers/card_processing.py::process_card(card_id: str)`. **This step's task body is a placeholder** (loads the card, no-ops) — it exists so `05-card-extraction` only has to fill in the actual vision-LLM call, not stand up the queue. **Amendment**: this step originally enqueued the task once per card immediately after upload (`process_card.delay(str(card.card_id))` inside `bulk_upload_cards`); that enqueue call was later removed and moved to the explicit `POST /cards/process` endpoint added in `05-parsing-visiting-card` — uploading a batch no longer triggers processing by itself.
...
## Files to change
- `apps/api/requirements.txt` — add `boto3`, `celery`, `redis`, `python-multipart`, `pillow-heif` (registers a Pillow plugin so `Image.open`/`.verify()` can decode HEIC/HEIF; Pillow itself bumped to the minimum version `pillow-heif` requires)
...
- `pillow-heif` — registers HEIC/HEIF as a Pillow-openable format (Pillow has no built-in decoder for it); `pillow_heif.register_heif_opener()` must run once per process, in every process that opens uploaded image bytes with Pillow — both the FastAPI process (`card_service.py`, upload-time validation) and the Celery worker process (`extraction_service.py`, `05-parsing-visiting-card`'s downscale step), since it's a process-local registration, not shared state
...
- The bulk-upload endpoint never triggers extraction itself and never waits on OCR — it returns as soon as each card's row is created and its image is stored. Triggering `process_card` is the explicit `POST /cards/process` endpoint's job (`05-parsing-visiting-card`), and that endpoint too must only enqueue and return, never wait on a vision-model call
...
- `EXPECTED_IMAGE_FORMATS` (the content-type → Pillow-`img.format` map used to confirm decoded bytes match the declared type) must stay in sync with `settings.allowed_card_image_content_types` — HEIC and HEIF both decode to Pillow's single `"HEIF"` format name (the `pillow-heif` plugin doesn't distinguish the two containers), so both content-types map to it
...
- [ ] `POST /cards/bulk-upload` with 3 valid JPEG files and no `exhibition_id` creates 3 `visiting_cards` rows (`status='new'`, `exhibition_id=NULL`), uploads 3 objects to the configured bucket, returns `201` with 3 card summaries, and enqueues **zero** `process_card` tasks (extraction only starts via the explicit `POST /cards/process` call)
- [ ] `POST /cards/bulk-upload` with a valid HEIC file (declared content-type `image/heic`, real HEIC bytes) is accepted exactly like a JPEG — creates its `visiting_cards` row and uploads the original HEIC bytes to storage (no server-side conversion at upload time), without enqueuing anything
